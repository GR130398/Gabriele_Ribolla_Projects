function [K,rho,feas,norm]=LMI_DT_DeDicontH2(Ftot,Gdec,Hdec,Htot,N,ContStruc,Gw,Dw,Du)
...
LMIconstr=[[P-Ftot*P*Ftot'-Ftot*L'*Gtot'-Gtot*L*Ftot'+Gw*Gw',Gtot*L;
            L'*Gtot', P;]>=1e-6*ones(2*ntot)];
LMIconstr = LMIconstr + [[S, Htot*P+Du*L; L'*Du' + P*Htot',P]>=1e-6*ones(2*ntot)];

% Gw' zeros(nw,ntot) normInfTFnoisesOutput*eye(nw,nw) Dw;
% zeros(ntot,ntot) Htot*P+Du*L Dw' normInfTFnoisesOutput*eye(ntot,ntot)]>=1e-10*ones(ntot+ntot+ mtot+ ntot,ntot+ntot+ mtot+ ntot)];
