...
MPC 
- mod : lin
- constraints : inputs + states
- weights : dinamic on the controur error
- cost : nl using splines 
- sim : FFD
%}
...
Xe_init   = [xRef2s(1,1);yRef2s(1,1);0];% (m) initial position in inertial axes
...
% nel MPC (considero tutto il circuito come way points)
y_way = yRef2s;
x_way = xRef2s;
...
% da riguardare la teoria del FHOCP per come settarle
...
%myoptions.outputfcn     =  abbiamo visto che mettendola la simulazione
%rallenta di molto 
...
Nsim = 100; % per limitare la lunghezza della simulazione

for ind = 2 : Nsim %%%%%%%%%%%%%%%%%% INIZIO MPC %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    fprintf('sim : %d \n',ind-1);


weight1_din = zeros(Np,2); % ad ogni iterazione lo ri-aggiorno in base al 
%progress sim dunque lo setto a zero
...
step_1_xy_sim(1:2,1) = Zsim_MPC(1:2,ind-1);
step_1_index_ref(:,1) = index_ref(1);
 desired_vec(:,1) = [x_des(1,1);y_des(1,1);Zsim_MPC(1:2,ind-1);index_ref(1)];
 ...
 [xstar,fxstar,niter,exitflag,xsequence] =...
     ...
     (x,th,th_cons,weight1_din,y_des,x_des,Zsim_MPC(:,ind-1),tx,ty,Ts,Np)...
...
u_sim_MPC(:,ind-1)= u; 
   ...
   zdot_MPC               =   linquadcopter_augmented_v1(0,Zsim_MPC(:,ind-1),u_sim_MPC(:,ind-1),th);
    Zsim_MPC(:,ind)       =   Zsim_MPC(:,ind-1)+Ts*zdot_MPC;



%% sim NP step in advance
...
%% Plot the results
x_sim       =   Zsim_MPC(1,1:ind)';
y_sim       =   Zsim_MPC(2,1:ind)';

figure,plot(y_sim,x_sim,'b.-',...
    y_way,x_way,'r.-')...
    ,grid on, hold on,xlabel('y'),ylabel('x')
legend('sim','des')
hold off
