function q_h = quad_nl_cost_constr_v2...
    (x,th,th_cons,y_des,x_des,z0,Ts,Np)

% non considera i constraints sugli stati
...
% optimization variables selection for the inputs 
...
% 
%     % r
%     z_sim(10,ind-1) = r_sim(1,1+floor(time_FFD(ind)/Ts));
%     % q
%     z_sim(11,ind-1) =   q_sim(1,1+floor(time_FFD(ind)/Ts));
%     %p
%     z_sim(12,ind-1) =   p_sim(1,1+floor(time_FFD(ind)/Ts));
%     % f1
...
r_sim       =   z_sim(10,1:Nblock:end)';
q_sim       =   z_sim(11,1:Nblock:end)';
p_sim       =   z_sim(12,1:Nblock:end)';
...
vetErroreContour = zeros(2,1);


for ind = 1 : nd 
p2 = [x_des(ind),y_des(ind)];
p1 = [x_sim(ind),y_sim(ind)];
matriceTang = funMatrixTang(p2,p1);
vetErroreContour = vetErroreContour + matriceTang * vetErroreTot;

end


cost2 = norm(vetErroreContour)^2;

errorLag = vetErroreTot - vetErroreContour;
cost1 = norm(errorLag)^2; % + norm(vetErroreContour)^2;

%% delta angular speeds cost 
cost3 = p_sim'*p_sim + q_sim'*q_sim + r_sim'*r_sim ; % data da tutti e tre

% altro modo per avere una piccola oscillazione delle input
...
% cost7 (in questo caso non considerato) weight7 = 0;
