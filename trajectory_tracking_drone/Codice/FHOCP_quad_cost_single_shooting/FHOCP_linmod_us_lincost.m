...
Xe_init   = [xRef2s(100,1);yRef2s(100,1);0];% (m) initial position in inertial axes
...
z_sim_MPC(:,1) =   z0; 
nu = 5;
u_sim_FHOCP            =   zeros(nu,tot_punti); % definizione vettore input 
...
speed_p_min = -20;% definita poi uguale a = -dist/Ts^2;
speed_p_max = 20;% definita poi uguale a = dist/Ts^2;
...
th_cons= [ weight1, weight2, weight3, weight4, weight5, weight6];
 ...
 [xstar,fxstar,niter,exitflag,xsequence] = ...
     myfmincon(@(x)f_quad_constr_FHOCP....
     (x,Ts,Np,th,th_cons,y_des,x_des,z0,desired_progress)...
     ,x0,[],[],C,d,p,q,myoptions);
...
for ind=2:Nsim_FFD
    u                  =   u_sim_FHOCP(:,1+floor(time_FFD(ind)/Ts));
